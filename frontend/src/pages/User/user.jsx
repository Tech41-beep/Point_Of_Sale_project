...
function User() {
 ...
 // useEffect: use for fetching users from the API
  useEffect(() => {
    async function fetchUsers() {
      ...
      try {
        ...
        setError(error.response?.data?.message || error.response?.data?.error || "Failed to fetch users");
      } finally {
        setLoading(false);
      }
    }
...
//delete handler
  async function handleDelete() {
    ...
    try {
      ...
      setError(requestError.response?.data?.message || requestError.response?.data?.error || "Failed to delete user");
    } finally {
      setDeletingId("");
    }
  }
  ...
  return (
    ...
    <div className="category-page rounded-xl bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <Link className="primary-button" to="/users/add">+ Add User</Link>
      </div>
    {/* search */}
          ...
          <tbody className="text-sm text-gray-700">
            {users.map((user) => (
              <tr key={user._id || user.id}>
                <td className="border-b border-gray-300 px-4 py-4 font-semibold">{user._id || user.id}</td>
                <td className="border-b border-gray-300 px-4 py-4">{user.name}</td>
                <td className="border-b border-gray-300 px-4 py-4">{user.email}</td>
                <td className="border-b border-gray-300 px-4 py-4">{user.role}</td>
                <td className="border-b border-gray-300 px-4 py-4">{user.note}</td>
            ...
            {/* loading and no users found messages */}
      ...
      {userToDelete && (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !deletingId) setUserToDelete(null);
          }}
        >
          <div
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p id="delete-user-description" className="mt-2 text-sm leading-6 text-gray-500">
                  You are about to permanently delete <span className="font-semibold text-gray-700">{userToDelete.name}</span>. This action cannot be undone.
                </p>
              </div>
            </div>
